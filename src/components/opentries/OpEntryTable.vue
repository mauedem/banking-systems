...
export default {
    name: "OpEntryTable", // Табличная форма "Операции"
    ...
    mounted() {
        if (this.selectedOpEntryRow) {
            const selectedRowIdx = this.opEntryList.findIndex(opEntry => opEntry.id === this.selectedOpEntryRow.id)
    ...
    methods: {
        selectRow(rowData) {
            ...
            if (!rowData.length) {
                this.$store.dispatch('opentry/setSelectedOpEntryRow', null)
        ...
        inputHandler(value, key) {
            const updatedOpEntryRow = this.$store.getters['opentry/updatedOpEntryRow']
        ...
        async selectTableRow(value) {
            ...
            const selectedRowIdx = this.opEntryList.findIndex(opEntry => opEntry.id === value.id)
